...
#end
{
		...
		if (hasMoreBelow()) {
			...
			while (counter < _slotsTotal) {
				if (styleB.el_scroll_time == 0) {
					...
					if (counter == 0)
						Reflect.setField(pr, "alpha", 0); // Fade the first element only
			...
			if (styleB.el_scroll_time == 0) {
				...
				r_el.alpha = 0;
				allTweens.push(FlxTween.tween(r_el, {alpha: 1, y: r_el.y - elementHeight}, styleB.el_scroll_time, {onComplete: __scrollComplete}));
			}
	...
	function animateAllSlots(Alpha:Array<Float>, StartOffs:Array<Int>, EndOffs:Array<Int>, onComplete:Void->Void) {
		...
		while (elementSlots[i] != null) {
			...
			allTweens.push(FlxTween.tween(elementSlots[i], {
				...
				alpha: Alpha[1]
			}, styleB.stw_el_time, {startDelay: (i * styleB.stw_el_wait), ease: easeFn}));
	...
	public function onScreen(focusAfter:Bool = true, ?onComplete:Void->Void) {
		visible = true;
		animateAllSlots([0, 1], styleB.stw_el_EnterOffs, [0, 0], function() {
			if (focusAfter)
				focus();
			if (onComplete != null)
				onComplete();
	...
	public function offScreen(?onComplete:Void->Void) {
		unfocus();
		animateAllSlots([0.9, 0], [0, 0], styleB.stw_el_ExitOffs, function() {
			visible = false;
			if (onComplete != null)
				onComplete();
		});
	...
	public function setPoolingMode(val:String, reuse_max:Int = DEF_POOL_REUSE_MAX) {
		switch (val) {
			...
			case 'reuse':
				...
				if (_pool_max_size < 1)
					_pool_max_size = DEF_POOL_REUSE_MAX;
			case 'off':
				flag_pool_recycle = false;
				flag_pool_reuse = false;
				_pool_max_size = 0;
			default:
				throw "Invalid pooling move";
		}
